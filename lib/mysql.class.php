...
class SafeMySQL
{
    ...
    private $defaults = array(
        ...
        'asObj'     => true, // if true it will return the results typecasted as objects insead of array
    );
    ...
    /**
     * decides what type of object should be returned
     * @param  array $value the data that will be returned
     * @return array|object        depending on the settings
     */
    ...
    private function prepareQuery($args)
    {
        ...
        // we check if we have named params
        preg_match("~:([A-Za-z0-9]+)~u", $raw, $match);
        $named = count($match);
        if($named)
            $vals = array_shift($args); // we grab the asociative array
        else
            $vals = array();

        /// echo "<pre>";print_r($array);
        $pnum  = floor(count($array) / 2) - floor(count($vals)/2); // we get the right number of params
